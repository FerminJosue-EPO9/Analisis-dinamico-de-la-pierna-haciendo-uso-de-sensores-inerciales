...
def calibrar_sensor(puerto, nombre):
    ...
    while True:
        try:
            ...
            print("\nHa ocurrido un problema durante la recolección de lecturas, intentándolo de nuevo...")
            time.sleep(5)
...
class LectorSensor(threading.Thread):
            ...
            try:
                ...
                while self.ejecutando:
                    ...
                    if accel is not None:
                        ...
                        termino_tangencial = np.cross(self.aceleracion_angular, self.r_vector)
                        termino_centripeto = np.cross(gyro, np.cross(gyro, self.r_vector))
                        self.aceleracion_scom = accel + termino_tangencial + termino_centripeto
    ...
    def get_datos_cinematicos(self):
        return self.rotacion_relativa, self.aceleracion_angular, self.aceleracion_scom
...
def main():
    ...
    try:
        while True:
            r_muslo, alpha_muslo, a_scom_m = hilo_muslo.get_datos_cinematicos()
            r_pantorrilla, alpha_pantorrilla, a_scom_p = hilo_pantorrilla.get_datos_cinematicos()
            ...
            if r_muslo is not None and r_pantorrilla is not None:
                ...
                flexion_knee = min(0.0, -ang_pantorrilla[1] - flexion_hip)
